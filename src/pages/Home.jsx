...
export default function Home() {
  ...
  // --- 1. LOGIQUE DE DATE (MOIS ACTUEL DU 1er AU 1er) ---
  const now = new Date();
  ...
  const totalPlannedExpenses = realExpenses + totalSubs;

  // --- 3. CALCUL GLOBAL (HISTORIQUE COMPLET) ---
  const totalIncomeAllTime = transactions.filter(t => t.amount > 0).reduce((acc, t) => acc + t.amount, 0);
  const totalExpenseAllTime = Math.abs(transactions.filter(t => t.amount < 0).reduce((acc, t) => acc + t.amount, 0));
  const totalBalance = totalIncomeAllTime - totalExpenseAllTime;

  // --- 4. DONNÉES GRAPHIQUES ---
  const barData = [
    { name: 'Entrées', amount: monthlyIncome, color: '#22c55e' },
    { name: 'Sorties + Abo', amount: totalPlannedExpenses, color: '#ef4444' }
  ];

  const pieData = CATEGORIES.map(cat => {
    // 1. Total des transactions réelles du mois pour cette catégorie
    const realTotal = Math.abs(monthlyTransactions
      .filter(t => t.categoryId === cat.id && t.amount < 0)
      .reduce((acc, t) => acc + t.amount, 0));

    // 2. Total des mensualités prévues pour cette même catégorie
    ...
    <div className="max-w-6xl mx-auto p-8 space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-purple-600 dark:bg-purple-700 p-8 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
            <p className="text-purple-200 font-bold uppercase tracking-widest text-xs">Solde Total Cumulé</p>
            <p className="text-7xl font-black mt-2 tracking-tighter">{totalBalance.toLocaleString()} €</p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6 transition-colors">
          <div>
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Entrées (Ce mois)</p>
            <p className="text-2xl font-black text-green-500">+{monthlyIncome.toLocaleString()} €</p>
          </div>
          <div className="pt-4 border-t border-slate-50 dark:border-slate-800">
            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">Prévu (Dépenses + Abo)</p>
            <p className="text-2xl font-black text-red-500">-{totalPlannedExpenses.toLocaleString()} €</p>
          </div>
        </div>
      </div>
      ...
      {hasAlerts && (
        <section className="animate-in slide-in-from-bottom-4 duration-500">
          <h3 className="text-xs font-bold uppercase text-slate-400 tracking-[0.2em] mb-4 ml-2">📅 Échéances & Prélèvements (30 jours)</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div key={sub.id} className="flex items-center justify-between p-4 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/50 rounded-3xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-blue-600 dark:text-blue-400 font-bold uppercase font-black">Mensuel</p>
                  </div>
                </div>
            ...
            {upcomingDebts.map(debt => (
              <div key={debt.id} className="flex items-center justify-between p-4 bg-orange-50/50 dark:bg-orange-900/10 border border-orange-100 dark:border-orange-800/50 rounded-3xl">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-orange-500 text-white rounded-xl flex items-center justify-center text-lg">{debt.type === 'to_pay' ? '💸' : '💰'}</div>
                  <div>
                    <p className="font-bold text-slate-900 dark:text-white text-sm">{debt.contact}</p>
                    <p className="text-[10px] text-orange-600 dark:text-orange-400 font-bold uppercase font-black">Dette • {debt.dueDate}</p>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm h-[400px]">
          <h3 className="font-bold text-slate-800 mb-6 uppercase tracking-wider text-sm dark:text-white">Flux du mois</h3>
        ...
        {/* Répartition catégorielle mensuelle */}
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm h-[400px] flex flex-col">
          <h3 className="font-bold text-slate-800 mb-6 uppercase tracking-wider text-sm dark:text-white">Dépenses par catégorie</h3>
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
